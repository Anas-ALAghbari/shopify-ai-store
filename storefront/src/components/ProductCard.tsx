  ...
  return (
    <div className="group bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition-all hover:shadow-2xl hover:shadow-blue-500/10 flex flex-col h-full">
      ...
      <div className="relative aspect-square overflow-hidden bg-slate-950">
        ...
        />
        {product.badge && (
          <span className="absolute top-3 right-3 bg-slate-950/80 backdrop-blur-md border border-slate-700/80 text-blue-300 text-xs font-bold px-3 py-1 rounded-full shadow-lg">
            {product.badge}
          </span>
        )}
      </div>
      ...
      <div className="p-5 flex flex-col flex-grow text-right">
        ...
        <div className="flex items-center justify-between text-xs text-slate-400 mb-2">
          ...
          <div className="flex items-center space-x-1 space-x-reverse text-amber-400">
        ...
        <div className="mt-auto pt-4 border-t border-slate-800 flex items-center justify-between">
          <div className="text-right">
            ...
            <span className="text-[10px] text-emerald-400 font-semibold">شحن شامل ومضمون</span>
          </div>

          <button
            onClick={() => onAddToCart(product)}
            className={`px-4 py-2.5 rounded-xl font-bold text-xs flex items-center space-x-2 space-x-reverse transition-all ${
              isAdded
